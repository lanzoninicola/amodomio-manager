        ...
        .map(p => {
            return {
                ...
                unitPrice: productsCompositions.filter(pc => pc.productId === p.id).reduce((acc, curr) => {
                    return acc + (ingredientsPrices.find(ip => ip.ingredientId === curr.componentId)?.unitPrice || 0)
                }, 0)
...
function CompositionElementForm({ productCompositionId, productId, componentId, componentType, unit, quantity, cost }: CompositionElementFormProps) {
    ...
    // this state is used to update the componentType when the componentId is changed
    const [elementIdSelected, setElementIdSelected] = useState(componentId)
                ...
                <div className="mb-4 w-full">
                    <div className="flex flex-col w-full">
                        <div className="flex flex-col w-full md:flex-row gap-2">
                            ...
                            <Input type="hidden" name="componentType" defaultValue={componentType || elementSelected?.type} />
                            <Input type="text" name="componentType" value={elementSelected?.unitPrice || 0} readOnly />
                            <Fieldset>
                                <Select name="componentId" defaultValue={componentId} required onValueChange={setElementIdSelected}>
                                    <SelectTrigger>
